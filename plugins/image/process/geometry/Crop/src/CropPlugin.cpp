...
int CropPlugin::safeModulo(const int& cropValue, const int& rodValue)
{
    if(rodValue == 0 || cropValue == 0)
    {
        return 0;
    }
    else
        return cropValue % int(rodValue * 0.5);
}
...
OfxRectI CropPlugin::computeCropRegion(const OfxTime time, const bool fromRatio) const
{
    ...
    switch(axis)
    {
        case eParamAxisXY:
        {
            ...
            if(symmetric == eParamSymmetricX || symmetric == eParamSymmetricXY)
            {
                cropRegion.x1 = safeModulo(cropRegion.x1, sRodSize.x);
                cropRegion.x2 = sRodSize.x - cropRegion.x1;
            }
            if(symmetric == eParamSymmetricY || symmetric == eParamSymmetricXY)
            {
                cropRegion.y1 = safeModulo(cropRegion.y1, sRodSize.y);
...
{
    //        TUTTLE_LOG_INFO( "changedClip:" << clipName );
...
{
    //        TUTTLE_LOG_INFO( "changedParam:" << paramName );
    ...
    //	}
    //	else
    if(paramName == kParamAxis || paramName == kParamSymmetric || paramName == kParamFixedRatio)
    {
    ...
    if(paramName == kParamXMin || paramName == kParamYMin || paramName == kParamXMax || paramName == kParamYMax ||
       paramName == kParamAxis || paramName == kParamSymmetric || paramName == kParamRatio)
    {

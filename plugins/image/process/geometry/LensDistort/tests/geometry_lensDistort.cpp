...
int main(int argc, char** argv)
{
    ...
    // png_write_view( "output_crop.png", color_converted_view<rgb8_pixel_t>( subimage_view(const_view(img_src), -250, -558,
    // img_src.dimensions().x+800, img_src.dimensions().y+1500 ) ) );
    ...
    tuttle::plugin::lens::resample_pixels<bilinear_sampler>(
        const_view(img_src), view(img_dst), static_cast<tuttle::plugin::lens::NormalLensDistortParams<double>*>(&_p),
        procWindow, &progress);

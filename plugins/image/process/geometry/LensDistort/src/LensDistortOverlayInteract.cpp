...
std::vector<std::vector<Point2> > createGrid(const OfxRectD& rod, const unsigned int nbTiles, const Point2& scale,
                                             const Point2& center, const unsigned int steps = 50)
{
...
{
    for(typename std::vector<std::vector<Point2> >::iterator itv = grid.begin(), itvEnd = grid.end(); itv != itvEnd; ++itv)
    {
        for(typename std::vector<Point2>::iterator it = itv->begin(), itEnd = itv->end(); it != itEnd; ++it)
        {
            *it += shift;
        }
    }
    ...
    , _interactScene(*effect, _infos)
{
    ...
    interact::PointInteract* centerPoint =
        new interact::ParamPoint<interact::FrameOptionalClip, eCoordinateSystemXXcn>(_infos, _plugin->_center, frame);
    _interactScene.push_back(new interact::ParamPointRelativePoint<interact::FrameOptionalClip, eCoordinateSystemXXcn>(
                                 _infos, _plugin->_gridCenter, frame, centerPoint),
                             new interact::IsActiveBooleanParamFunctor<>(_plugin->_gridCenterOverlay));
...
bool LensDistortOverlayInteract::draw(const OFX::DrawArgs& args)
{
    ...
    if(_plugin->_gridOverlay->getValue() && _plugin->_clipSrc->isConnected())
    {
        ...
        if(!_plugin->_displaySource->getValue())
        {
            ...
            static const double pixelAspectRatio =
                1.0; // here the pixel aspect ratio is 1.0 because we work in canonical coordinates

...
inline Obj transformValues(const EParamLensType lensType, const LensDistortProcessParams<double>& params, const Obj& obj)
{
    ...
    BOOST_THROW_EXCEPTION(exception::Unsupported() << exception::user("Unknown lens type."));
    return obj;
}
...
{
    for(typename std::vector<boost::gil::point2<F2> >::iterator it = vec.begin(), itEnd = vec.end(); it != itEnd; ++it)
    {
        *it = params.apply(*it);
    }
}

...
BOOST_FORCEINLINE void copy_pixels(const View& src, const OfxRectI& srcRegion, const View& dst, const OfxRectI& dstRegion)
{
    ...
    View srcCrop = subimage_view(src, srcRegion.x1, srcRegion.y1, regionSize.x, regionSize.y);
    View dstCrop = subimage_view(dst, dstRegion.x1, dstRegion.y1, regionSize.x, regionSize.y);

    copy_pixels(srcCrop, dstCrop);
}

template <class View>
void fillAroundIntersection(const View& viewA, const OfxRectI& srcAPixelRod, const View& viewB, const OfxRectI& srcBPixelRod,
                            const View& dstView, const OfxRectI& dstPixelRod, const OfxRectI& procWindowRoW)
{
...
template <class View>
void fillAroundIntersection(const View& viewA, const OfxRectI& srcAPixelRod, const OfxRectI& srcBPixelRod,
                            const View& dstView, const OfxRectI& dstPixelRod, const OfxRectI& procWindowRoW)
{
    // fill with A
...
void MergeProcess<View, Functor>::multiThreadProcessImages(const OfxRectI& procWindowRoW)
{
    ...
    switch(_params._rod)
    {
        ...
        case eParamRodUnion:
        {
            fillAroundIntersection(this->_srcViewA, srcRodA, this->_srcViewB, srcRodB, this->_dstView, this->_dstPixelRod,
                                   procWindowRoW);
            break;
        }
        case eParamRodA:
        {
            fillAroundIntersection(this->_srcViewA, srcRodA, srcRodB, this->_dstView, this->_dstPixelRod, procWindowRoW);
            break;
        }
        case eParamRodB:
        {
            fillAroundIntersection(this->_srcViewB, srcRodB, srcRodA, this->_dstView, this->_dstPixelRod, procWindowRoW);
            break;
        }
    }
    ...
    View dstView_inter = subimage_view(this->_dstView, procIntersect.x1 - this->_dstPixelRod.x1,
                                       procIntersect.y1 - this->_dstPixelRod.y1, procIntersectSize.x, procIntersectSize.y);

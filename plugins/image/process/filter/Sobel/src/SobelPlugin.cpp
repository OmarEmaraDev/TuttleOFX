...
template <template <class, class> class Process, bool sPlanar, class SLayout, class SBits, bool dPlanar, class DLayout,
          class DBits, class Plugin>
    ...
    switch(dBitDepth)
    {
        ...
        case OFX::eBitDepthNone:
        {
            BOOST_THROW_EXCEPTION(exception::Unsupported() << exception::user() + "Bit depth (" +
                                                                  mapBitDepthEnumToString(dBitDepth) +
...
void localDoGilRender2(Plugin& plugin, const OFX::RenderArguments& args, const OFX::EPixelComponent dComponent,
                       const OFX::EBitDepth dBitDepth)
{
    switch(dComponent)
    {
        ...
        {
            BOOST_THROW_EXCEPTION(exception::Unsupported() << exception::user() + "Pixel component (" +
                                                                  mapPixelComponentEnumToString(dComponent) +
...
void localDoGilRender2(Plugin& plugin, const OFX::RenderArguments& args, const bool dPlanar,
                       const OFX::EPixelComponent dComponent, const OFX::EBitDepth dBitDepth)
{
    if(dPlanar)
    {
...
void localDoGilRender2(Plugin& plugin, const OFX::RenderArguments& args, const OFX::EBitDepth sBitDepth, const bool dPlanar,
                       const OFX::EPixelComponent dComponent, const OFX::EBitDepth dBitDepth)
{
    switch(sBitDepth)
    {
        ...
        {
            BOOST_THROW_EXCEPTION(exception::Unsupported() << exception::user() + "Bit depth (" +
                                                                  mapBitDepthEnumToString(sBitDepth) +
                                                                  ") not recognized by the plugin.");
        }
    }
    BOOST_THROW_EXCEPTION(exception::Unknown());
...
void localDoGilRender2(Plugin& plugin, const OFX::RenderArguments& args, const OFX::EPixelComponent sComponent,
                       const OFX::EBitDepth sBitDepth, const bool dPlanar, const OFX::EPixelComponent dComponent,
                       const OFX::EBitDepth dBitDepth)
{
    switch(sComponent)
    {
        case OFX::ePixelComponentRGBA:
        {
            localDoGilRender2<Process, sPlanar, boost::gil::rgba_layout_t>(plugin, args, sBitDepth, dPlanar, dComponent,
                                                                           dBitDepth);
            return;
        }
        case OFX::ePixelComponentRGB:
        {
            localDoGilRender2<Process, sPlanar, boost::gil::rgb_layout_t>(plugin, args, sBitDepth, dPlanar, dComponent,
                                                                          dBitDepth);
            return;
        }
        case OFX::ePixelComponentAlpha:
        {
            localDoGilRender2<Process, sPlanar, boost::gil::gray_layout_t>(plugin, args, sBitDepth, dPlanar, dComponent,
                                                                           dBitDepth);
            return;
        }
        case OFX::ePixelComponentCustom:
        ...
        {
            BOOST_THROW_EXCEPTION(exception::Unsupported() << exception::user() + "Pixel component (" +
                                                                  mapPixelComponentEnumToString(sComponent) +
                                                                  ") not supported by the plugin.");
        }
    }
}
...
void localDoGilRender2(Plugin& plugin, const OFX::RenderArguments& args, const bool sPlanar,
                       const OFX::EPixelComponent sComponent, const OFX::EBitDepth sBitDepth, const bool dPlanar,
                       const OFX::EPixelComponent dComponent, const OFX::EBitDepth dBitDepth)
{
    if(sPlanar)
    {
...
{
    localDoGilRender2<Process, Plugin>(plugin, args, false, sClip.getPixelComponents(), sClip.getPixelDepth(), false,
                                       dClip.getPixelComponents(), dClip.getPixelDepth());
}
}
}
...
SobelProcessParams<SobelPlugin::Scalar> SobelPlugin::getProcessParams(const OfxPointD& renderScale) const
{
    ...
    params._xKernelGaussianDerivative =
        buildGaussianDerivative1DKernel<Scalar>(params._size.x, normalizedKernel, kernelEpsilon);
    ...
    {
        params._yKernelGaussianDerivative =
            buildGaussianDerivative1DKernel<Scalar>(params._size.y, normalizedKernel, kernelEpsilon);
        if(!params._unidimensional)
...
void SobelPlugin::changedParam(const OFX::InstanceChangedArgs& args, const std::string& paramName)
{
    if(paramName == kParamInfos)
    {
        ...
        infos << "  for X (" << params._xKernelGaussian.size() << "x" << params._xKernelGaussianDerivative.size() << ")"
              << std::endl;
        infos << "  for Y (" << params._yKernelGaussian.size() << "x" << params._yKernelGaussianDerivative.size() << ")"
              << std::endl;
        infos << std::endl;
        infos << "X :" << std::endl;
...
bool SobelPlugin::getRegionOfDefinition(const OFX::RegionOfDefinitionArguments& args, OfxRectD& rod)
{
    ...
    switch(params._border)
    {
        case eParamBorderPadded:
            rod.x1 =
                srcRod.x1 + std::max(params._xKernelGaussianDerivative.left_size(), params._yKernelGaussian.left_size());
            rod.y1 =
                srcRod.y1 + std::max(params._xKernelGaussian.left_size(), params._yKernelGaussianDerivative.left_size());
            rod.x2 =
                srcRod.x2 - std::max(params._xKernelGaussianDerivative.right_size(), params._yKernelGaussian.right_size());
            rod.y2 =
                srcRod.y2 - std::max(params._xKernelGaussian.right_size(), params._yKernelGaussianDerivative.right_size());
            return true;
        default:
            break;
    }

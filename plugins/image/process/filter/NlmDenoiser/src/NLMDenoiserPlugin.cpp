...
void NLMDenoiserPlugin::getFramesNeeded(const OFX::FramesNeededArguments& args, OFX::FramesNeededSetter& frames)
{
    ...
    //	TUTTLE_LOG_INFO( "NLMDenoiserPlugin::getFramesNeeded timerange min:" << realRange.min << ", max:" << realRange.max <<
    //" for time:" << args.time );
}
...
void NLMDenoiserPlugin::getRegionsOfInterest(const OFX::RegionsOfInterestArguments& args, OFX::RegionOfInterestSetter& rois)
{
    ...
    const OfxRectD roi = {double(args.regionOfInterest.x1 - margin), double(args.regionOfInterest.y1 - margin),
                          double(args.regionOfInterest.x2 + margin + 1), double(args.regionOfInterest.y2 + margin + 1)};
    ...
    switch(dstComponents)
    {
        case OFX::ePixelComponentRGBA:
        {
            switch(dstBitDepth)
            {
                ...
                {
                    TUTTLE_LOG_ERROR("Bit depth (" << mapBitDepthEnumToString(dstBitDepth)

...
AVReaderPlugin::AVReaderPlugin(OfxImageEffectHandle handle)
    ...
    , _paramMetaDataCustom(common::kPrefixMetaData, AV_OPT_FLAG_DECODING_PARAM, false)
...
void AVReaderPlugin::ensureVideoIsOpen()
{
    ...
    if(_lastInputFilePath == filepath && !_lastInputFilePath.empty() && _lastVideoStreamIndex == videoStreamIndex)
        return;
    ...
    {
        cleanInputFile();
        BOOST_THROW_EXCEPTION(exception::FileNotExist() << exception::filename(filepath));
    }
    ...
    try
    {
        ...
        _inputStream =
            &_inputFile->getStream(_inputFile->getProperties().getVideoProperties().at(videoStreamIndex).getStreamIndex());
    ...
    {
        cleanInputFile();
        BOOST_THROW_EXCEPTION(exception::Failed() << exception::user() + "unable to open input file : " + e.what()
...
void AVReaderPlugin::changedParam(const OFX::InstanceChangedArgs& args, const std::string& paramName)
{
    ...
    else if(paramName == kTuttlePluginFilename)
    {
        ...
        try
        {
            ...
            BOOST_FOREACH(const avtranscoder::SubtitleProperties& subtitleStream,
                          params._inputProperties->getSubtitleProperties())
            {
            ...
            BOOST_FOREACH(const avtranscoder::AttachementProperties& attachementStream,
                          params._inputProperties->getAttachementProperties())
            {
            ...
            BOOST_FOREACH(const avtranscoder::UnknownProperties& unknownStream,
                          params._inputProperties->getUnknownProperties())
            {
            ...
            common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerOutputFormat, params._inputFormatName,
                                                     common::kPrefixFormat);
            // update video details parameters
            const std::string videoCodecName = params._inputVideoProperties->getCodecName();
            common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerVideoCodec, videoCodecName,
                                                     common::kPrefixVideo);
        }
        catch(std::exception& e)
        {
            ...
            common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerOutputFormat, "",
                                                     common::kPrefixFormat);
            common::disableOFXParamsForFormatOrCodec(*this, _libavFeatures._optionsPerVideoCodec, "", common::kPrefixVideo);
        }
    }
...
void AVReaderPlugin::beginSequenceRender(const OFX::BeginSequenceRenderArguments& args)
{
    ...
    const avtranscoder::ProfileLoader::Profile formatDetailProfile =
        _paramFormatDetailCustom.getCorrespondingProfile(params._inputFormatName);
    ...
    const avtranscoder::ProfileLoader::Profile videoDetailProfile =
        _paramVideoDetailCustom.getCorrespondingProfile(params._inputVideoProperties->getCodecName());

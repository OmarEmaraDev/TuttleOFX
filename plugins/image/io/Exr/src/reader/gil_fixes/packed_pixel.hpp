...
/// \brief A heterogeneous color base whose elements are reference proxies to channels in a pixel. Models
/// ColorBaseValueConcept. This class is used to model packed pixels, such as 16-bit packed RGB.
...
/// \brief Heterogeneous pixel value whose channel references can be constructed from the pixel bitfield and their index.
/// Models ColorBaseValueConcept, PixelValueConcept, PixelBasedConcept
/// Typical use for this is a model of a packed pixel (like 565 RGB)
template <typename BitField, // A type that holds the bits of the pixel. Typically an integral type, like uint16_t
          typename ChannelRefVec, // An MPL vector whose elements are packed channels. They must be constructible from
...
struct packed_pixel
{
    ...
    packed_pixel() {}
    explicit packed_pixel(const BitField& bitfield)
        : _bitfield(bitfield)
    {
    }
...
inline typename kth_element_reference_type<packed_pixel<P, C, L>, K>::type at_c(packed_pixel<P, C, L>& p)
{
    return typename kth_element_reference_type<packed_pixel<P, C, L>, K>::type(&p._bitfield);
}

template <int K, typename P, typename C, typename L>
inline typename kth_element_const_reference_type<packed_pixel<P, C, L>, K>::type at_c(const packed_pixel<P, C, L>& p)
{
...
/// The pointer packed_pixel<P,CR,Layout>* is used as an iterator over interleaved pixels of packed format. Models

...
public:
    ...
    ColorGrandient2DLinearFunctor(const point_t& size, const std::vector<point_t>& points,
                                  const std::vector<value_type>& colors)
        : _points(points)
        , _colors(colors)
    {
        ...
        for(PointsVector::iterator it = _points.begin(), itEnd = _points.end(); it != itEnd; ++it)
        {
...
private:
    ...
    void computeDistances(const point_t& p) const
    {
        ...
        for(; it_dist != itEnd_dist; ++it_dist, ++it_point)
        {
            *it_dist = 1.0 / distance(p, *it_point);
            sum += *it_dist;
        }
        for(it_dist = _distances.begin(); it_dist != itEnd_dist; ++it_dist)
        {
...
public:
    result_type operator()(const point_t& p) const
    {
        ...
        for(; it_color != itEnd_color; ++it_color, ++it_dist)
        {
            // outputColor += color * dist
            pixel_plus_assign_t<result_type, result_type>()(
                pixel_multiplies_scalar_t<result_type, Scalar, result_type>()(*it_color, *it_dist), outputColor);
        }
        return outputColor;
    }
};

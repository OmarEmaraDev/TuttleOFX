...
void DebugImageEffectApiPlugin::changedParam(const OFX::InstanceChangedArgs& args, const std::string& paramName)
{
    ...
    else if(paramName == kUpdate)
    {
        ...
        for(OFX::ImageEffectHostDescription::PixelComponentArray::const_iterator it = desc->_supportedComponents.begin(),
                                                                                 itEnd = desc->_supportedComponents.end();
            it != itEnd; ++it)
            description << OFX::mapPixelComponentEnumToString(*it) << ", ";
        description << std::endl;
        description << "_supportedContexts: ";
        for(OFX::ImageEffectHostDescription::ContextArray::const_iterator it = desc->_supportedContexts.begin(),
                                                                          itEnd = desc->_supportedContexts.end();
            it != itEnd; ++it)
            description << OFX::mapContextEnumToString(*it) << ", ";
        description << std::endl;
        description << "_supportedPixelDepths: ";
        for(OFX::ImageEffectHostDescription::BitDepthArray::const_iterator it = desc->_supportedPixelDepths.begin(),
                                                                           itEnd = desc->_supportedPixelDepths.end();
            it != itEnd; ++it)

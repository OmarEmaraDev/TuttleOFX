...
std::shared_ptr<attribute::Image> loadImage(const std::string& imagePath)
{
    ...
    compute(outputCache, list_of(NodeInit(io::getBestReader(imagePath)).setParam("filename", imagePath.c_str())));
...
loadAndGenerateThumbnail(const std::string& imagePath, const std::string& thumbnailToCreate, const int thumbnailMaxSize)
{
    ...
    std::vector<INode*> nodes =
        graph.addConnectedNodes(list_of(NodeInit(io::getBestReader(imagePath)).setParam("filename", imagePath.c_str()))(
            NodeInit("tuttle.resize").setParam("size", thumbnailMaxSize, thumbnailMaxSize).setParam("keepRatio", true))(
            NodeInit(io::getBestWriter(thumbnailToCreate)).setParam("filename", thumbnailToCreate.c_str())));
    graph.setup();
    ...
    graph.compute(outputCache, NodeListArg(), cOptions, internCache);
    return outputCache.get(0);
}

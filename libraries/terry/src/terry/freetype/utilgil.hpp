...
inline void copy_and_convert_alpha_blended_pixels(const GlyphView& glyphView, const typename View::value_type& glyphColor,
                                                  const View& dstView)
{
...
inline void wuline(const view_t& view, const value_type& pixel, int X0, int Y0, int X1, int Y1, int NumLevels,
                   int IntensityBits)
{
    ...
    if(DeltaY > DeltaX)
    {
        ...
        while(--DeltaY)
        {
            ...
            boost::gil::static_for_each(dst, view(X0, Y0), make_alpha_blend((Weighting ^ WeightingComplementMask)));
            view(X0, Y0) = dst;

            dst = pixel;
            boost::gil::static_for_each(dst, view(X0 + XDir, Y0), make_alpha_blend(Weighting));
            view(X0 + XDir, Y0) = dst;
        }
    ...
    while(--DeltaX)
    {
        ...
        dst = pixel;
        boost::gil::static_for_each(dst, view(X0, Y0 + 1), make_alpha_blend(Weighting));
        view(X0, Y0 + 1) = dst;
    }
...
struct draw_line
{
    ...
    {
        wuline(view, pixel, pt0.x, pt0.y, pt1.x, pt1.y, NumLevels, IntensityBits);
    }
};
}

...
struct make_advance_width
{
    ...
    void operator()(FT_Glyph_Metrics metrics, int kerning) { advance += kerning + (metrics.horiAdvance >> 6); }
};
...
public:
    ...
    void operator()(const glyph_t& glyph, int kerning = 0)
    {
        ...
        if(glyphRegionSize.x != 0 && glyphRegionSize.y != 0)
        {
            ...
            gray8c_view_t glyphView = interleaved_view(width, height, reinterpret_cast<gray8_pixel_t*>(slot->bitmap.buffer),

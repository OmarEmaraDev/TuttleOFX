...
struct grayMean_t
{
    ...
    PixelDst operator()(const PixelSrc& src) const
    {
        ...
        get_color(dst, gray_color_t()) =
            1.f / 3 * (get_color(src, red_t()) + get_color(src, green_t()) + get_color(src, blue_t()));
        return dst;
    }
};
...
struct grayRec601_t
{
    ...
    PixelDst operator()(const PixelSrc& src) const
    {
        ...
        get_color(dst, gray_color_t()) =
            (0.299f * get_color(src, red_t()) + 0.587f * get_color(src, green_t()) + 0.114f * get_color(src, blue_t()));
        return dst;
    }
};
...
struct grayRec709_t
{
    ...
    PixelDst operator()(const PixelSrc& src) const
    {
        ...
        get_color(dst, gray_color_t()) =
            (0.2126f * get_color(src, red_t()) + 0.7152f * get_color(src, green_t()) + 0.0722f * get_color(src, blue_t()));
        return dst;
    }
};
...
DView& convertPremultipliedView(SView& src, DView& dst, bool premultiplied = false)
{
    ...
    if(premultiplied)
    {
        transform_pixels(src, dst, premutliplied_t<typename SView::value_type, typename DView::value_type>());
    }
    else
    {
        transform_pixels(src, dst, notPremutliplied_t<typename SView::value_type, typename DView::value_type>());
    }
    return dst;
}
...
DView& convertPremultipliedRGBAView(SView& src, DView& dst, bool premultiplied = false)
{
    ...
    if(premultiplied)
    {
        transform_pixels(src, dst, premutlipliedRGBA_t<typename SView::value_type, typename DView::value_type>());
    }
    else
    {
        transform_pixels(src, dst, notPremutlipliedRGBA_t<typename SView::value_type, typename DView::value_type>());
    }
    return dst;
}
...
DView& convertToGrayView(SView& src, DView& dst, EConvertToGray& grayMethod, bool premultiplied = false)
{
    ...
    switch(grayMethod)
    {
        case eConvertToGrayMean:
            transform_pixels(src, dst, grayMean_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGrayRec601:
            transform_pixels(src, dst, grayRec601_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGrayRec709:
            transform_pixels(src, dst, grayRec709_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGraySelectRed:
            transform_pixels(src, dst, grayFromRed_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGraySelectGreen:
            transform_pixels(src, dst, grayFromGreen_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGraySelectBlue:
            transform_pixels(src, dst, grayFromBlue_t<typename SView::value_type, typename DView::value_type>());
            break;
        case eConvertToGraySelectAlpha:
            transform_pixels(src, dst, grayFromAlpha_t<typename SView::value_type, typename DView::value_type>());
            break;
    }
    return dst;
}
...
DView& addAlphaChannelOnView(SView& src, DView& dst, bool premultiplied = false)
{
    ...
    transform_pixels(src, dst, rgbToRgba_t<typename SView::value_type, typename DView::value_type>());
    return dst;
}
...
DView& convertToRgbViewAndAddAlpha(SView& src, DView& dst, bool unpremultiplied = false)
{
    ...
    transform_pixels(src, dst, grayToRgba_t<typename SView::value_type, typename DView::value_type>());
    return dst;
}
...
DView& convertToRgbView(SView& src, DView& dst, bool unpremultiplied = false)
{
    ...
    transform_pixels(src, dst, grayToRgb_t<typename SView::value_type, typename DView::value_type>());
    return dst;
}
...
template <>
void convertComponentsView<rgba32f_view_t, rgba32f_view_t>(rgba32f_view_t& src, rgba32f_view_t& dst,
...
template <>
void convertComponentsView<rgba16_view_t, rgba16_view_t>(rgba16_view_t& src, rgba16_view_t& dst,
                                                         ConvertionParameters& parameters)
{
    convertPremultipliedRGBAView(src, dst, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgba8_view_t, rgba8_view_t>(rgba8_view_t& src, rgba8_view_t& dst,
                                                       ConvertionParameters& parameters)
{
    convertPremultipliedRGBAView(src, dst, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgba32f_view_t, rgb32f_view_t>(rgba32f_view_t& src, rgb32f_view_t& dst,
                                                          ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<rgba16_view_t, rgb16_view_t>(rgba16_view_t& src, rgb16_view_t& dst,
                                                        ConvertionParameters& parameters)
{
    convertPremultipliedView(src, dst, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgba32f_view_t, gray32f_view_t>(rgba32f_view_t& src, gray32f_view_t& dst,
                                                           ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<rgba16_view_t, gray16_view_t>(rgba16_view_t& src, gray16_view_t& dst,
                                                         ConvertionParameters& parameters)
{
    convertToGrayView(src, dst, parameters.grayMethod, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgba8_view_t, gray8_view_t>(rgba8_view_t& src, gray8_view_t& dst,
                                                       ConvertionParameters& parameters)
{
    convertToGrayView(src, dst, parameters.grayMethod, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgb32f_view_t, rgba32f_view_t>(rgb32f_view_t& src, rgba32f_view_t& dst,
                                                          ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<rgb16_view_t, rgba16_view_t>(rgb16_view_t& src, rgba16_view_t& dst,
                                                        ConvertionParameters& parameters)
{
    addAlphaChannelOnView(src, dst, parameters.premultiplied);
}
...
template <>
void convertComponentsView<rgb32f_view_t, gray32f_view_t>(rgb32f_view_t& src, gray32f_view_t& dst,
                                                          ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<rgb16_view_t, gray16_view_t>(rgb16_view_t& src, gray16_view_t& dst,
                                                        ConvertionParameters& parameters)
{
    convertToGrayView(src, dst, parameters.grayMethod);
}
...
template <>
void convertComponentsView<gray32f_view_t, rgba32f_view_t>(gray32f_view_t& src, rgba32f_view_t& dst,
                                                           ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<gray16_view_t, rgba16_view_t>(gray16_view_t& src, rgba16_view_t& dst,
                                                         ConvertionParameters& parameters)
{
    convertToRgbViewAndAddAlpha(src, dst);
}
...
template <>
void convertComponentsView<gray8_view_t, rgba8_view_t>(gray8_view_t& src, rgba8_view_t& dst,
                                                       ConvertionParameters& parameters)
{
    convertToRgbViewAndAddAlpha(src, dst);
}
...
template <>
void convertComponentsView<gray32f_view_t, rgb32f_view_t>(gray32f_view_t& src, rgb32f_view_t& dst,
                                                          ConvertionParameters& parameters)
{
...
template <>
void convertComponentsView<gray16_view_t, rgb16_view_t>(gray16_view_t& src, rgb16_view_t& dst,
                                                        ConvertionParameters& parameters)
{
    convertToRgbView(src, dst);
}

...
private:
    // this static array of pointers to member variables makes operator[] safe and doesn't seem to exhibit any performance
    // penalty
...
inline bool pointInRect(const Point& p, const Rect& rec)
{
    ...
    return p.x >= orientedRec.x1 && p.x <= orientedRec.x2 && p.y >= orientedRec.y1 && p.y <= orientedRec.y2;
}

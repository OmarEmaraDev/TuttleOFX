...
struct pixel_plus_assign_t
{
    ...
    {
        static_for_each(
            p1, p2, channel_plus_assign_t<typename channel_type<PixelSrc>::type, typename channel_type<PixelDst>::type>());
        return p2;
    }
};
...
{
    GIL_FORCEINLINE
    typename channel_traits<ChannelDst>::reference operator()(const Scalar s,
...
struct pixel_multiplies_scalar_assign_t
{
    ...
    {
        static_for_each(
            p, std::bind1st(channel_multiplies_scalar_assign_t<Scalar, typename channel_type<PixelDst>::type>(), s));
        return p;
    }
};

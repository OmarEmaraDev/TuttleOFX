...
struct pixel_pow_t
{
    ...
    PixelR operator()(const PixelRef& p) const
    {
        ...
        static_transform(p, result,
                         channel_pow_t<typename channel_type<PixelRef>::type, n, typename channel_type<PixelR>::type>());
        return result;
    }
};
...
struct pixel_scalar_pow_t
{
    ...
    PixelR operator()(const PixelRef& p, const Scalar& s) const
    {
        ...
        static_transform(
            p, result,
            std::bind2nd(
                channel_scalar_pow_t<typename channel_type<PixelRef>::type, Scalar, typename channel_type<PixelR>::type>(),
                s));
        return result;
    }
};

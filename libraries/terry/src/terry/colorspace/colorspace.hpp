...
struct color_dependencies
{
    ...
    typedef typename ::boost::mpl::push_back<typename color_dependencies<typename color::reference>::from_root, color>::type
        from_root;
    /// list of color types dependencies to root (eg. HSL: [HSL,RGB,XYZ])
    typedef typename ::boost::mpl::push_front<typename color_dependencies<typename color::reference>::to_root, color>::type
        to_root;
    ...
    typedef typename ::boost::mpl::zip_view<
        ::boost::mpl::vector<typename details::from_root_A, typename details::from_root_B> >::type color_steps_from_root;
    /// a vector of colors pairs (eg. HSL: [(HSL,RGB),(RGB,XYZ)])
    typedef typename ::boost::mpl::zip_view<
        ::boost::mpl::vector<typename details::to_root_A, typename details::to_root_B> >::type color_steps_to_root;
};
template <>
...
struct FullColorParams
{
    ...
    typedef typename ::boost::mpl::transform<from_root, color_to_colorparams_mplfunc< ::boost::mpl::_1> >::type
        full_params; ///< all params from root to Color
     ...
     */
    std::size_t getNbReferences() const { return size::value; }
};
 ...
 * @param ChannelType type used from pixels (defined the bitdepth used: uint8_t, float32_t, ...)
...
struct color_transformation_types_impl
{
    ...
    static const std::size_t dIndex = ::boost::static_unsigned_min < fromRoot ? sIndex + 1 : sIndex - 1,
                             indexMax > ::value; // incrementation depends from the direction
    static const std::size_t nextIndex = ::boost::static_unsigned_min < fromRoot ? dIndex + 1 : dIndex - 1,
                             indexMax > ::value;
    ...
    typedef typename ::boost::mpl::at_c<ColorsFromRoot, dIndex>::type
        DColor; // dest color index (destination for current color_transformation_step)
    typedef typename ::boost::mpl::at_c<ColorsFromRoot, nextIndex>::type NextColor; // next color index
    typedef
        typename ::boost::mpl::back<ColorsFromRoot>::type Color; // color from which the ColorsFromRoot mpl::vector is build
    typedef typename ::boost::mpl::if_c<fromRoot, typename ::boost::mpl::back<ColorsFromRoot>::type,
                                        typename ::boost::mpl::front<ColorsFromRoot>::type>::type
        FColor; // final destination color
...
struct color_transformation_static_for_impl_rec
{
    ...
    void operator()(const typename T::FullParams& params, const typename T::SPixel& src, typename T::DPixel& dst,
                    typename T::FPixel& finalDst) const
    {
        BOOST_STATIC_ASSERT(sizeLeft > 3);

        color_transformation_step(boost::fusion::at_c<T::index>(params._params), src, dst);

        typename T::NextPixel pixNext;

        color_transformation_static_for_impl_rec<ColorsFromRoot, sizeLeft - 1, ChannelType, fromRoot>()(params, dst, pixNext,
                                                                                                        finalDst);
    }
};
...
struct color_transformation_static_for_impl_rec<ColorsFromRoot, 3, ChannelType, fromRoot>
{
                    ...
                    typename T::FPixel& finalDst) const
    {
        ...
        color_transformation_step(boost::fusion::at_c<T::colorParamIndex>(params._params), // A
                                  src, dst);

        color_transformation_step(boost::fusion::at_c<T::nextColorParamIndex>(params._params), // B
                                  dst, finalDst);
    }
};
...
struct color_transformation_static_for_impl
{
    ...
    void operator()(const typename T::FullParams& params, const typename T::SPixel& src, typename T::FPixel& dst) const
    {
        typename T::DPixel pixNext;

        // multiple steps in a static foreach color_transformation_step
        color_transformation_static_for_impl_rec<ColorsFromRoot, colorsSize, ChannelType, fromRoot>()(params, src, pixNext,
                                                                                                      dst);
    }
};
...
struct color_transformation_static_for_impl<ColorsFromRoot, 2, ChannelType, fromRoot>
{
    ...
    {
        // only one step
        color_transformation_step(boost::fusion::at_c<T::colorParamIndex>(params._params), src, dst);
    }
};
template <class ColorsFromRoot, class ChannelType, bool fromRoot>
...
struct color_transformation_static_for
{
    ...
    void operator()(const typename T::FullParams& params, const typename T::SPixel& src, typename T::FPixel& dst) const
    {
        color_transformation_static_for_impl<ColorsFromRoot, sizeLeft, ChannelType, fromRoot>()(params, src, dst);
    }
};
...
void color_transformation_to_root(const FullColorParams<Color>& params,
                                  const boost::gil::pixel<ChannelType, typename Color::layout>& src,
                                  boost::gil::pixel<ChannelType, XYZ::layout>& dst)
{
    color_transformation_static_for<typename FullColorParams<Color>::from_root, ChannelType, false>()(params, src, dst);
}

/**
 */
template <class Color, typename ChannelType>
void color_transformation_from_root(const boost::gil::pixel<ChannelType, XYZ::layout>& src,
                                    const FullColorParams<Color>& params,
                                    boost::gil::pixel<ChannelType, typename Color::layout>& dst)
{
    color_transformation_static_for<typename FullColorParams<Color>::from_root, ChannelType, true>()(params, src, dst);
}
...
void color_transformation(const FullColorParams<SColor>& sParams,

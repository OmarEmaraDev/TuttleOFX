...
bool modifyVectors(const View& xVecView, const View& yVecView, const double angle, const double intensity, Progress& p)
{
    ...
    for(int y = 0; y < xVecView.height(); ++y)
    {
        ...
        for(; it_xVec != itEnd_xVec; ++it_xVec, ++it_yVec)
        {
            VecPoint2 gradient;
...
bool correlateMotionVectors(GView& xGradientView, GView& yGradientView, View& img, const Point& topleft,
                            ...
                            const convolve_boundary_option boundary_option, Progress& p)
{
    typedef typename GView::value_type GPixel;
    using namespace boost::gil;
    correlate_rows_cols<GPixel, Alloc>(color_converted_view<GPixel>(img), kernel, kernelSecondary, xGradientView, topleft,
                                       boundary_option);
    if(p.progressForward(xGradientView.size()))
        return true;
    correlate_rows_cols<GPixel, Alloc>(color_converted_view<GPixel>(img), kernelSecondary, kernel, yGradientView, topleft,
                                       boundary_option);
    if(p.progressForward(yGradientView.size()))
        return true;
    return false;
...
template <typename Sampler, // Models SamplerConcept
          ...
          typename DstView, typename Progress>
// Models MutableRandomAccess2DImageViewConcept
bool motionvectors_resample_pixels(const SrcView& srcView, const Rect<std::ssize_t>& srcRod, const VecView& xVecView,
                                   const VecView& yVecView, const Rect<std::ssize_t>& vecRod, const DstView& dstView,
                                   const Rect<std::ssize_t>& dstRod, const Rect<std::ssize_t>& procWindowRoW,
                                   const sampler::EParamFilterOutOfImage outOfImageProcess, Progress& p,
                                   Sampler sampler = Sampler())
{
    ...
    for(Coord y = procWindowRoW.y1; y < procWindowRoW.y2; ++y)
    {
        ...
        for(Coord x = procWindowRoW.x1; x < procWindowRoW.x2; ++x, ++xit_dst, ++xit_xVec, ++xit_yVec)
        {
            const Coord xSrc = x - srcRod.x1;
            const VecPoint2 pos(xSrc, ySrc);

            VecPoint2 motion;
            if(x < vecRod.x1 || x > vecRod.x2 || y < vecRod.y1 || y > vecRod.y2)
            {
                motion.x = 0;
                motion.y = 0;
            }

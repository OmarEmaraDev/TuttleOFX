...
/**
 * @brief fill all pixels respecting the @p condition in a range of pixels (on the same line or with an 1d_traversable
 * image).
 */
...
void flood_fill(const SView& srcView, const Rect<std::ssize_t>& srcRod, DView& dstView, const Rect<std::ssize_t>& dstRod,
                const Rect<std::ssize_t>& procWindow, const StrongTest& strongTest, const SoftTest& softTest)
{
    ...
    for(std::ssize_t y = procWindow.y1; y < procWindow.y2; ++y)
    {
        {
            ...
            for(std::ssize_t x = procWindow.x1; x < procWindow.x2; ++x, ++dst_loc.x(), ++src_loc.x())
            {
                    ...
                    if(/*inside &&*/ containsStrong)
                    {
                        ...
                        if(y < rod.y2)
                        {
                            ...
                            fill_pixels_range_if(srcLastBegin.x_at(-Connexity::x, 1), src_loc.x_at(Connexity::x, 1),
                                                 dstLastBegin.x_at(-Connexity::x, 1), white, softTest);
                        }
                    }
                    // re-init values
                    inside = false;
        ...
        while(!propagation.empty())
        {
            ...
            if(iElem._direction == eDirectionBellow && iElem._y > y + 1)
                continue;
            ...
            // x propagation
            // left
            if(Connexity::x || softTest((*gElem._srcBegin)[0])) // check first
            {
                while(gElem._xBegin - 1 >= rod.x1 && softTest((*(gElem._srcBegin.x() - 1))[0]))
                {
                    --(gElem._xBegin);
                    --(gElem._srcBegin.x());
                    --(gElem._dstBegin.x());
            ...
            // right
            if(Connexity::x || softTest((*(gElem._srcEnd.x() - 1))[0])) // check last
            {
                while(gElem._xEnd < rod.x2 && softTest((*gElem._srcEnd)[0]))
                {
                    ++(gElem._xEnd);
                    ++(gElem._srcEnd.x());
                }
            ...
            // new sub-ranges
            {
                ...
                for(std::ssize_t xx = gElem._xBegin; xx < gElem._xEnd; ++xx)
                {
...
void flood_fill_bruteForce(const SView& srcView, const Rect<std::ssize_t>& srcRod, DView& dstView,
                           const Rect<std::ssize_t>& dstRod, const Rect<std::ssize_t>& procWindow,
                           const StrongTest& strongTest, const SoftTest& softTest)
{
    ...
    {
        for(int x = procWindow.x1; x < procWindow.x2; ++x, ++dst_loc.x(), ++src_loc.x())
        {
    ...
    //	{
    DView tmp_dst = subimage_view(dstView, procWindowOutput.x1, procWindowOutput.y1, procWindowSize.x, procWindowSize.y);
...
void applyFloodFill(const SView& srcView, DView& dstView, const double lowerThres, const double upperThres)
{
    ...
    terry::algorithm::transform_pixels(localView, minmax);
    ...
    floodFill::flood_fill<floodFill::Connexity4, floodFill::IsUpper<Scalar>, floodFill::IsUpper<Scalar>, SView, DView,
                          Allocator>(srcView, getBounds<std::ptrdiff_t>(srcView), dstView,
                                     getBounds<std::ptrdiff_t>(dstView),
                                     rectangleReduce(getBounds<std::ptrdiff_t>(dstView), 1),
                                     floodFill::IsUpper<Scalar>(upperThresR), floodFill::IsUpper<Scalar>(lowerThresR));
}
}
}

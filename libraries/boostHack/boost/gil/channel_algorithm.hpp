...
template <int K>
struct unsigned_integral_num_bits<packed_channel_value<K> > : public mpl::int_<K>
{
};
...
When creating new channel models, it is often a good idea to provide specializations for the channel conversion algorithms,
for
example, for performance optimizations. If the new model is an integral type that can be signed, it is easier to define the
conversion
only for the unsigned type (\p channel_converter_unsigned) and provide specializations of \p
detail::channel_convert_to_unsigned
...
struct channel_converter_unsigned
    : public detail::channel_converter_unsigned_impl<SrcChannelV, DstChannelV, is_integral<SrcChannelV>::value,
...
struct channel_converter_unsigned_integral_impl<uintmax_t, DstChannelV, false, true>
{
    DstChannelV operator()(uintmax_t src) const
    {
        static const uintmax_t div =
            unsigned_integral_max_value<uint32_t>::value / unsigned_integral_max_value<DstChannelV>::value;
...
struct channel_converter_unsigned_integral_impl<SrcChannelV, DstChannelV, SrcLessThanDst, false>
    : public channel_converter_unsigned_integral_nondivisible<
          ...
          mpl::greater<mpl::plus<unsigned_integral_num_bits<SrcChannelV>, unsigned_integral_num_bits<DstChannelV> >,
...
struct channel_converter_unsigned_integral_nondivisible<SrcChannelV, DstChannelV, true, true>
{
    DstChannelV operator()(SrcChannelV src) const
    {
        static const double mul =
            unsigned_integral_max_value<DstChannelV>::value / double(unsigned_integral_max_value<SrcChannelV>::value);
        return DstChannelV(src * mul);
    }
};
...
template <>
struct channel_converter_unsigned<uint32_t, float32_t> : public std::unary_function<uint32_t, float32_t>
{
    float32_t operator()(uint32_t x) const
    {
        ...
        // uint32_t matches max_value of float32_t
...
/// \brief 32 bit <-> float channel conversion
template <>
struct channel_converter_unsigned<float32_t, uint32_t> : public std::unary_function<float32_t, uint32_t>
{
...
template <>
struct channel_convert_to_unsigned<int8_t> : public std::unary_function<int8_t, uint8_t>
{
    typedef uint8_t type;
    type operator()(int8_t val) const { return val + 128; }
};

template <>
struct channel_convert_to_unsigned<int16_t> : public std::unary_function<int16_t, uint16_t>
{
    typedef uint16_t type;
    type operator()(int16_t val) const { return val + 32768; }
};

template <>
struct channel_convert_to_unsigned<int32_t> : public std::unary_function<int32_t, uint32_t>
{
    typedef uint32_t type;
    type operator()(int32_t x) const { return static_cast<uint32_t>(x + (1 << 31)); }
};
...
template <>
struct channel_convert_from_unsigned<int8_t> : public std::unary_function<uint8_t, int8_t>
{
    typedef int8_t type;
    type operator()(uint8_t val) const { return val - 128; }
};

template <>
struct channel_convert_from_unsigned<int16_t> : public std::unary_function<uint16_t, int16_t>
{
    typedef int16_t type;
    type operator()(uint16_t val) const { return val - 32768; }
};

template <>
struct channel_convert_from_unsigned<int32_t> : public std::unary_function<uint32_t, int32_t>
{
    typedef int32_t type;
    type operator()(uint32_t x) const { return static_cast<int32_t>(x - (1 << 31)); }
};
...
struct channel_converter : public std::unary_function<SrcChannelV, DstChannelV>
{
    DstChannelV operator()(const SrcChannelV& src) const
    {
        ...
        typedef channel_converter_unsigned<typename to_unsigned::result_type, typename from_unsigned::argument_type>
            converter_unsigned;
...
template <>
struct channel_multiplier_unsigned<uint8_t> : public std::binary_function<uint8_t, uint8_t, uint8_t>
{
...
template <>
struct channel_multiplier_unsigned<uint16_t> : public std::binary_function<uint16_t, uint16_t, uint16_t>
{

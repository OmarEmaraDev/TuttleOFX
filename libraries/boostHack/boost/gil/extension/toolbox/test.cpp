...
struct mandelbrot_fn
{
    ...
    result_type operator()(const point_t& p) const
    {
        ...
        double t =
            get_num_iter(point2<double>(p.x / (double)_img_size.x * 3 - 2, p.y / (double)_img_size.y * 3 - 1.0f)); // 1.5f));
        t = pow(t, 0.2);
...
template <class PIXEL, class VIEW>
inline PIXEL min_channel_values_(const VIEW& view, const size_t percent = 1)
{
...
template <class PIXEL, class VIEW>
inline PIXEL max_channel_values(const VIEW& view)
{
    PIXEL max;
    ...
    // find the max values
    for(VIEW::iterator it = view.begin(); it != view.end(); ++it)
    {
        for(int i = 0; i < num_channels<PIXEL>::type::value; ++i)
        {
...
template <class PIXEL, class VIEW>
inline boost::fusion::vector<PIXEL, PIXEL> minmax_channel_values(const VIEW& view)
{
    PIXEL min, max;
...
{
    template <typename CHANNEL>
    void operator()(CHANNEL& diff, const CHANNEL& min, const CHANNEL& max) const
    {
        diff = max - min;
    }
};
    ...
    for(int y = 0; y < src_view.height(); ++y)
    {
        ...
        for(int x = 0; x < src_view.width(); ++x)
        {
            ...
            for(int i = 0; i < num_channels<src_pixel_t>::type::value; ++i)
            {
                ...
                {
                    dynamic_at_c(dst, i) = static_cast<dst_channel_t>(
                        dst_max * (static_cast<float>(dynamic_at_c(src, i) - dynamic_at_c(min, i)) /
...
struct foo
{
    ...
    void operator()(DST_CHANNEL& dst, SRC_CHANNEL& src, SRC_CHANNEL& min, SRC_CHANNEL& diff) const
    {
        ...
        float d = (static_cast<float>(_dst_max) *
                   ((static_cast<float>(src) - static_cast<float>(min)) / static_cast<float>(diff)));
...
struct do_it
{
    ...
    {
        static_for_each(*_dst_it, src, _min, _diff, _op);
        ++_dst_it;
    }
...
int main(int argc, char* argv[])
{
...

    {
        ...
        my_virt_view_t mandel(dims, locator_t(point_t(0, 0), point_t(1, 1),
                                              deref_t(dims, src_pixel_t(max_value, 0, 0), src_pixel_t(0, max_value, 0))));
        ...
        down_sample(view(img), view(eight_bit_img));

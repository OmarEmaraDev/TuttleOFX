...
// used for virtual locator
template <typename IndicesLoc, typename PaletteLoc, typename Enable = void // there is specilization for integral indices
          >
struct indexed_image_deref_fn : indexed_image_deref_fn_base<IndicesLoc, PaletteLoc>
{
    ...
    indexed_image_deref_fn(const indices_locator_t& indices_loc, const palette_locator_t& palette_loc)
        : indexed_image_deref_fn_base(indices_loc, palette_loc)
    {
    }

    result_type operator()(const point_t& p) const { return *_palette_loc.xy_at(at_c<0>(*_indices_loc.xy_at(p)), 0); }
};
    ...
    : indexed_image_deref_fn_base<IndicesLoc, PaletteLoc>
{
    ...
    result_type operator()(const point_t& p) const { return *_palette_loc.xy_at(*_indices_loc.xy_at(p), 0); }
};

template <typename IndicesLoc, typename PaletteLoc>
struct indexed_image_locator_type
{
    typedef virtual_2d_locator<indexed_image_deref_fn<IndicesLoc, PaletteLoc>, false> type;
};
...
public:
    ...
    const palette_view_t get_palette_view() const { return palette_view_t(point_t(num_colors(), 1), palette()); }
...
template <typename Index, typename Pixel, typename IndicesAllocator = std::allocator<unsigned char>,
...
public:
    ...
    typedef typename indexed_image_locator_type<indices_locator_t, palette_locator_t>::type locator_t;
    ...
    indexed_image(const x_coord_t width = 0, const y_coord_t height = 0, const std::size_t num_colors = 1,
                  const std::size_t indices_alignment = 0, const std::size_t palette_alignment = 0)
    ...
    indexed_image(const point_t& dimensions, const std::size_t num_colors = 1, const std::size_t indices_alignment = 0,
...
private:
    ...
    {
        typedef indexed_image_deref_fn<indices_locator_t, palette_locator_t> defer_fn_t;
...
template <typename Index, typename Pixel>
inline const typename indexed_image<Index, Pixel>::const_view_t const_view(indexed_image<Index, Pixel>& img)
{

...
struct make_advance_width
{
    ...
    void operator()(FT_Glyph_Metrics metrics) { advance += metrics.horiAdvance >> 6; }
};
...
struct render_glyphs
{
    int x;
    const view_t& view;
    render_glyphs(const view_t& view)
        : view(view)
        , x(0)
    {
    }
    ...
    void operator()(glyph& glyph)
    {
        ...
        gray8c_view_t gray =
            interleaved_view(width, height, (gray8_pixel_t*)glyphslot->bitmap.buffer, glyphslot->bitmap.width);
...
struct text
{
    ...
    text(FTC_Manager manager, std::string str, color_t color, int id, int size, int align = center | middle, int options = 0,
         char special = '#')
        : manager(manager)
        , str(str)
        , id(id)
    ...
    text(FTC_Manager manager, color_t color, int id, int size, int align = center | middle, int options = 0,
         char special = '.')
        : manager(manager)
        , id(id)
        , size(size)
    ...
    void operator()(view_t& view)
    {
        ...
        if(options & clipspecial)
        {
            ...
            if(pos >= 1 && pos < glyphs.size())
            {
                ...
                while(true)
                {
                    ...
                    std::transform(glyphs2.begin(), glyphs2.end(), std::back_inserter(metrics), make_metric());

                    std::vector<FT_Glyph_Metrics>::iterator it =
                        std::find_if(metrics.begin(), metrics.end(), find_last_fitted_glyph(view.width()));
        ...
        std::vector<FT_Glyph_Metrics>::iterator it = std::find_if(m.begin(), m.end(), find_last_fitted_glyph(view.width()));
...
struct elipsed_layer
{
    ...
    void operator()(view_t& view)
    {
        ...
        text_t lback(manager, sback, color, id, size, text_t::right | text_t::bottom);
        view_t vback = view;
        lback(vback);

        text_t lfront(manager, sfront, color, id, size, text_t::left | text_t::bottom);
        int width = view.width() - vback.width() - dwidth;
        view_t vfront = subimage_view(view, 0, 0, width, view.height());
        lfront(vfront);

        text_t ldots(manager, color, id, size, text_t::center | text_t::bottom, text_t::fill, '.');
